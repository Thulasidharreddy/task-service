/*
 * This file is generated by jOOQ.
 */
package com.neurasim.jooq.performance_schema.tables.records;


import com.neurasim.jooq.performance_schema.tables.CondInstances;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record2;
import org.jooq.Row2;
import org.jooq.impl.UpdatableRecordImpl;
import org.jooq.types.ULong;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class CondInstancesRecord extends UpdatableRecordImpl<CondInstancesRecord> implements Record2<String, ULong> {

    private static final long serialVersionUID = 1L;

    /**
     * Setter for <code>performance_schema.cond_instances.NAME</code>.
     */
    public CondInstancesRecord setName(String value) {
        set(0, value);
        return this;
    }

    /**
     * Getter for <code>performance_schema.cond_instances.NAME</code>.
     */
    public String getName() {
        return (String) get(0);
    }

    /**
     * Setter for
     * <code>performance_schema.cond_instances.OBJECT_INSTANCE_BEGIN</code>.
     */
    public CondInstancesRecord setObjectInstanceBegin(ULong value) {
        set(1, value);
        return this;
    }

    /**
     * Getter for
     * <code>performance_schema.cond_instances.OBJECT_INSTANCE_BEGIN</code>.
     */
    public ULong getObjectInstanceBegin() {
        return (ULong) get(1);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    public Record1<ULong> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record2 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row2<String, ULong> fieldsRow() {
        return (Row2) super.fieldsRow();
    }

    @Override
    public Row2<String, ULong> valuesRow() {
        return (Row2) super.valuesRow();
    }

    @Override
    public Field<String> field1() {
        return CondInstances.COND_INSTANCES.NAME;
    }

    @Override
    public Field<ULong> field2() {
        return CondInstances.COND_INSTANCES.OBJECT_INSTANCE_BEGIN;
    }

    @Override
    public String component1() {
        return getName();
    }

    @Override
    public ULong component2() {
        return getObjectInstanceBegin();
    }

    @Override
    public String value1() {
        return getName();
    }

    @Override
    public ULong value2() {
        return getObjectInstanceBegin();
    }

    @Override
    public CondInstancesRecord value1(String value) {
        setName(value);
        return this;
    }

    @Override
    public CondInstancesRecord value2(ULong value) {
        setObjectInstanceBegin(value);
        return this;
    }

    @Override
    public CondInstancesRecord values(String value1, ULong value2) {
        value1(value1);
        value2(value2);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached CondInstancesRecord
     */
    public CondInstancesRecord() {
        super(CondInstances.COND_INSTANCES);
    }

    /**
     * Create a detached, initialised CondInstancesRecord
     */
    public CondInstancesRecord(String name, ULong objectInstanceBegin) {
        super(CondInstances.COND_INSTANCES);

        setName(name);
        setObjectInstanceBegin(objectInstanceBegin);
    }

    /**
     * Create a detached, initialised CondInstancesRecord
     */
    public CondInstancesRecord(com.neurasim.jooq.performance_schema.tables.pojos.CondInstances value) {
        super(CondInstances.COND_INSTANCES);

        if (value != null) {
            setName(value.getName());
            setObjectInstanceBegin(value.getObjectInstanceBegin());
        }
    }
}
